<template>
  <div class="page-container">
    ...
    <div>
      ...
      <v-btn v-if="hasMoreData" @click="bottom=true" flat small color="primary">Load Next</v-btn>
    </div>
...
export default {
  ...
  methods: {
    ...
    setupListeners() {
      ...
      eventBus.$on(
        this.sqldata.processId + "runsql3SingleRecordDataScroll",
        data => {
          this.bottomVisibleNew(data);
        }
      );
    },
    //-----------------------------------------------
    turnOffListeners() {
      ...
      eventBus.$off(this.sqldata.processId + "runsql3SingleRecordDataScroll");
    },
